...
6  Appendix ................................................................................35
6.1  Appendix A - Management of Serial Line Diagnostic Counters 35
...
(cid:3)    In  unicast  mode,  the  master  addresses  an  individual  slave.    After  receiving  and  processing  the  request,  the  slave  returns  a
...
(cid:3)  In broadcast mode, the master can send a request to all slaves.
...
(cid:1)  On MODBUS Serial Line, the Address field only contains the slave address.
...
(cid:1)  The function code indicates to the server what kind of action to perform.  The function code can be followed by a data field that

contains request and response parameters.

(cid:1)  Error checking field is the result of a "Redundancy Checking" calculation that is performed on the message contents.  Two kinds
...
(cid:1)  State "Idle" = no pending request.  This is the initial state after power-up. A request can only be sent in "Idle" state.  After sending

a request, the Master leaves the "Idle" state, and cannot send a second request at the same time

(cid:1)  When a unicast request is sent to a slave, the master goes into "Waiting for reply" state, and a “Response Time-out” is started.  It
prevents  the  Master  from  staying  indefinitely  in  "Waiting  for  reply"  state.    Value  of  the  Response  time-out  is  application
dependant.

(cid:1)  When a reply is received, the Master checks the reply before starting the data processing.  The checking may result in an error,
...
(cid:1)  If no reply is received, the Response time-out expires, and an error is generated.  Then the Master goes into "Idle" state, enabling
...
(cid:1)  When a broadcast request is sent on the serial bus, no response is returned from the slaves. Nevertheless a delay is respected
...
(cid:1)  In  unicast  the  Response  time  out  must  be  set  long  enough  for  any  slave  to  process  the  request  and  return  the  response,  in
...
(cid:1)  Frame error consists of : 1) Parity checking applied to each character; 2) Redundancy checking applied to the entire frame.  See
...
(cid:1)  State "Idle" = no pending request.  This is the initial state after power-up.
(cid:1)  When a request is received, the slave checks the packet before performing the action requested in the packet.  Different errors

may occur :  format error in the request, invalid action, …  In case of error, a reply must be sent to the master.

(cid:1)  Once the required action has been completed, a unicast message requires that a reply must be formatted and sent to the master.
(cid:1)  If the slave detects an error in the received frame, no respond is returned to the master.
(cid:1)  MODBUS  diagnostics  counters  are  defined  and  should  be  managed  by  any  slave  in  order  to  provide  diagnostic  information.
...
Remarks :
(cid:1)  the  duration  of  the  REQUEST,  REPLY,  BROACAST  phases  depends  on  the  communication  features  (frame  length  and

throughput).

(cid:1)  the duration of the WAIT and TREATMENT phases depends on the request processing time needed for the slave application.
...
(cid:3)  The maximum size of a MODBUS RTU frame is 256 bytes.
...
Comment
If frame OK
(cid:1) processing frame
If frame NOK
(cid:1) delete entire frame
...
Some explanations about the above state diagram:
(cid:1)  Transition from "Initial State" to "Idle" state needs t3.5 time-out expiration :  that insures inter-frame delay
(cid:1)  "Idle" state is the normal state when neither emission nor reception is active.
(cid:1)  In RTU mode, the communication link is declared in "idle" state when there is no transmission activity after a time interval equal to

at least 3,5 characters.

(cid:1)  When the link is in idle state, each transmitted character detected on the link is identified as the start of a frame. The link goes to
...
(cid:1)  After detection of the end of frame, the CRC calculation and checking is completed. Afterwards the address field is analysed to
...
(cid:3) Example : The byte 0X5B is encoded as two characters : 0x35 and 0x42 ( 0x35 ="5", and 0x42 ="B" in ASCII ).
...
Some explanations about the above state diagram :
(cid:1)  "Idle" state is the normal state when neither emission nor reception is active.
(cid:1)  Each reception of a ":" character means a beginning of a new message.  If a message was in process of reception while receiving

such a character, the current message is declared incomplete and it is discarded. A new reception buffer is then allocated.

(cid:1)  After detection of the end of frame, the LRC calculation and checking is completed. Afterwards the address field is analyzed to
determine  if  the  frame  is  for  the  device.  If  not  the  frame  is  discarded.    In  order  to  reduce  the  reception  processing  time  the
...
(cid:1)  Parity checking (even or odd) should be applied to each character.
(cid:1)  Frame checking (LRC or CRC) must be applied to the entire message.
...
(cid:1)  In RTU mode, messages include an error–checking field that is based on a Cyclical Redundancy Checking (CRC) method. The
...
(cid:1)  In ASCII mode, messages include an error–checking field that is based on a Longitudinal Redundancy Checking (LRC) method.
...
(cid:1)  the device integrates the communication transceiver and is connected to the trunk using a Passive Tap and a derivation cable

( case of Slave 1 and Master ) ;

(cid:1)  the device doesn't integrate the communication transceiver and is connected to the trunk using an Active Tap and a derivation

cable (the active TAP integrates the transceiver)
( case of Slave 2 ) ;

(cid:1)  the device is connected directly to the trunk cable, in a Daisy-Chain ( case of Slave n )

The following conventions are adopted :

(cid:1)  The interface with the trunk is named ITr  (Trunk Interface)
(cid:1)  The interface between the device and the Passive Tap is named IDv  (Derivation Interface)
(cid:1)  The interface between the device and the Active Tap is named AUI  (Attachment Unit Interface)

Remarks :

1.

In some cases, the Tap may be connected directly to the IDv-socket or the AUI-socket of the device, without using a derivation
cable.
...
(cid:1)  Power Supply :
(cid:1)  Port mode control :  PMC  circuit  (  TTL  compatible  ).    When  needed,  port  mode  may  be  controlled  either  by  this  external
...
(cid:1)  Power Supply :
(cid:1)  PMC circuit :  See above ( In 2W-MODBUS Circuits Definition ) the note about this optional circuit.
...
(cid:1)  TxD0 signal shall be wired with the RxD0 signal, turning them to the D0 signal
(cid:1)  TxD1 signal shall be wired with the RxD1 signal, turning them to the D1 signal.
(cid:1)  Pull-up, Pull-down and line terminations resistors shall be re-arranged to correctly adapt the D0, D1 signals.
...
(cid:1)  TxD0 signal shall be wired with the RxD0 signal and then connected to the D0 signal of the trunk ;
(cid:1)  TxD1 signal shall be wired with the RxD1 signal and then connected to the D1 signal of the trunk.
...
(cid:1)  All the implemented requests.
(cid:1)  The operating modes.
(cid:1)  The visual diagnostics.
(cid:1)  The reachable registers and supported function codes.
(cid:1)  Installation rules.

(cid:1)  The required information in the following sections should also be documented :
...
(cid:1)  A specific indication relating to the devices addresses, is to be written in the form of an important warning :
...
(cid:1)  A "Getting Started" chapter is highly recommended, with the documented description of a typical application example, for an
...
RS485 2W-cabling (and 4W-cabling as an
additional option)
